...
<div class="w3-main w3-content" style="max-width:1600px;margin-top:83px">
    <p>CLICK ON A GAME IMAGE TO BUY A GAME</p>
      <div class="w3-row w3-grayscale-min">  
  <!-- Photo grid -->
<?php
    
    $sql="SELECT title,imager.path from game,imager where game.gid=imager.gid";
    if($result=mysqli_query($conn,$sql)){
      while($row=mysqli_fetch_row($result)){  ?>

      <div class="w3-quarter">
      <a href="/vlib/buy.php?id=<?php echo $row[0];?>"><img src=<?php echo $row[1]; ?> style="width:100%" onclick="onClick(this)" alt=<?php echo $row[0];?>>
      </div>
